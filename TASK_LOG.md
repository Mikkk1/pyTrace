...
[2026-06-16] BUG FIX — Live Mode scope isolation: traceStore.ts setMode('live') now resets result/steps/totalSteps/currentStepIndex/currentStep/error/liveError/notes so Trace Mode data doesn't bleed into Live Mode; setMode('trace') clears liveError to remove stale error banners
[2026-06-16] BUG FIX — Live Mode inputs bar hidden: App.tsx wraps TestCaseInput + Run button div in {mode === 'trace'}, hiding the entire Inputs bar in Live Mode (eliminates "Invalid JSON" error from the empty inputs field); debounced auto-rerun now passes {} instead of stale inputs state, and drops the inputsError/inputs===null guards
[2026-06-16] BUG FIX — Live Mode display error: App.tsx displayError = mode === 'live' ? liveError : (inputsError ?? error), so inputsError from the hidden Inputs bar no longer surfaces in Live Mode
[2026-06-16] BUG FIX — Variable deduplication (out-of-scope collections): VariablePanel.tsx buildRows now calls collectionSummaryLabel on removed-row values and sets isGroupHeader=true for non-null summary, so out-of-scope collections show as faded badge rows (e.g. "nums Array[3]" with strikethrough) instead of full expanded values; VarRow group-header branch gains opacity-35 + line-through label styling when row.isOutOfScope is true; tsc --noEmit -p tsconfig.app.json clean
[2026-06-16] BUG FIX — Live Mode clear on empty editor (Bug 3): App.tsx live debounce now checks code.trim() and scans for executable lines (non-empty, non-comment); if the editor is blank or comment-only, reset() is called immediately without hitting the backend — clearing COLLECTIONS/VARIABLES/CALL STACK instantly; removed the `code || DEFAULT_CODE` fallback so empty Live Mode stays blank
[2026-06-16] BUG FIX — traceStore reset() + mode switch (Bugs 3/5): reset() now also clears liveError; setMode() now unconditionally resets ALL visualization state (result/steps/totalSteps/currentStepIndex/currentStep/error/liveError/notes) on BOTH Live→Trace and Trace→Live switches — no stale data from either mode carries over
[2026-06-16] BUG FIX — Clear button in Live Mode editor (Bug 4): CodeEditor.tsx imports Trash2 from lucide-react; when mode === 'live', renders an absolute-positioned Trash2 button (top-right, z-10) that calls setCode('') + reset() on click; tooltip "Clear editor and visualization"; button only visible in Live Mode; wrapped MonacoEditor in a relative h-full div
[2026-06-16] BUG 1 VERIFIED: Inputs bar already correctly wrapped in {mode === 'trace' && ...} in App.tsx from previous session — TestCaseInput and Run button are fully hidden in Live Mode
[2026-06-16] BUG 2 VERIFIED: Preprocessor _auto_add_call_site() already generates full-argument call sites — confirmed with direct tests: two_sum(nums, target), search(nums, target), characterReplacement(s, k) all correct; no code change needed; tsc --noEmit -p tsconfig.app.json clean
[2026-06-16] BUG FIX — Trace Mode scratchpad stale inputs: App.tsx handleRun now detects whether code has any `def` at the start of a line; scratchpad code (no function def) runs with {} as inputs instead of stale TestCaseInput values, so backend build_restricted_globals receives no pre-seeded vars; function code still uses the TestCaseInput inputs with the inputsError/inputs===null guard; tsc --noEmit clean
[2026-06-16] BUG FIX — Preprocessor arg extraction rewrite (Bug 1): _auto_add_call_site now uses regex r'def\s+\w+\s*\(([^)]*)\)' to parse params from the cleaned def line instead of relying solely on AST args.args; strips type annotations (split on ':'), separates required vs default params (split on '='), filters self/cls, generates required params first then optional; falls back to AST for multi-line defs; all 5 required signatures verified: two_sum(nums,target), minWindow(s,t), characterReplacement(s,k), search(nums,target), climbStairs(n)
[2026-06-16] BUG FIX — t defaults to null in TestCaseInput (Bug 2): added 't': '"ABC"' to DEFAULTS so minWindow(self,s,t) gets t="ABC" by default instead of null/None; with t being a non-empty string, it now appears in COLLECTIONS as a char grid alongside s
[2026-06-16] BUG FIX — Empty dicts missing from COLLECTIONS/VARIABLES (Bug 3): collections.ts kindOf() now returns 'dict' for ALL plain dicts including empty ones (removed the > 0 guard); collectionSummaryLabel() returns 'Dict{0}' for empty dicts so VariablePanel shows a summary badge row instead of raw '{}'; DictVisualizer renders an italic 'empty' placeholder row when the dict has no entries; need={} and window={} now appear in COLLECTIONS from the moment they are created
[2026-06-16] PHASE-8 SECTION-1: preprocessor.py _auto_add_call_site() rewritten to use pure AST param extraction — func_nodes[0].args.args gives clean parameter names after annotation stripping; removed all regex-based param extraction; also fixed _fix_call_site() bug where last_line inside a function body (indented) was mistakenly treated as a wrong call site (e.g. float("inf") being replaced with coinChange("inf")); all 8 preprocessor signature tests pass (minWindow, twoSum, climbStairs, numIslands, canFinish, solve, characterReplacement, rob)
[2026-06-16] PHASE-8 SECTION-2: sandbox.py — pre-seeded build_restricted_globals with common DSA imports: Counter, defaultdict, deque, OrderedDict (collections), heapq, math, bisect, string, itertools, functools modules, and typing shims (List, Dict, Set, Tuple, Optional, Union, Any, inf); users can now use Counter/heapq/defaultdict without explicit imports; MAX_STEPS raised 10_000 -> 50_000; EXECUTION_TIMEOUT raised 5 -> 8s
[2026-06-16] PHASE-8 SECTION-3: tracer.py — added defaultdict serialization as {"__type__":"defaultdict","items":{...}} before generic dict branch; added float('inf')/'NaN' serialization as string sentinels ("Infinity", "-Infinity", "NaN") to avoid invalid JSON; set sys.setrecursionlimit(500) before exec() (restored after); collections.ts — added DefaultDictObj interface + isDefaultDictObj, 'defaultdict' CollectionKind + KIND_ORDER entry, kindOf/collectionSummaryLabel handlers; CollectionsPanel.tsx — 'defaultdict' case renders DictVisualizer with "defaultdict" badge; VariablePanel.tsx — float sentinels display as ∞/-∞/NaN with numeric color
[2026-06-16] PHASE-8 SECTION-6: collections.ts — added q, bfs_queue, dfs_stack, explored to ROLE_BADGES so queues named q show "queue" badge and dfs stacks show "stack" badge
[2026-06-16] PHASE-8 SECTION-8: errorHints.ts — replaced flat ERROR_HINTS dict with ordered ERROR_HINT_PATTERNS array; sub-message patterns via RegExp allow specific hints (e.g. 'NoneType' AttributeError, 'not subscriptable' TypeError, float('inf') ValueError); 12 error types covered (IndexError, KeyError, TypeError, NameError, AttributeError, RecursionError, ValueError, ZeroDivisionError, OverflowError, StopIteration, MemoryError, TimeoutError)
[2026-06-16] PHASE-8 SECTION-10: All 4 new NeetCode patterns tested and pass — topKFrequent (Counter+heap, 22 steps), rob (1D DP, 19 steps), canFinish (defaultdict+deque graph BFS, 33 steps), coinChange (float-inf DP, 54 steps); all Phase 7 tests still pass (5/5: twoSum, lengthOfLongestSubstring, characterReplacement, minWindow, numIslands)
